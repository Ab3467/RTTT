...
function App() {
  ...
  let gameBoard = [...Board.map((array) => [...array])]; //updating object state immutably
  for (let turn of gameturn) {
    const { square, Player } = turn;    // player is X or O and square is object with row or col
  ...
  for (const combination of WINNING_COMBINATION) {
    ...
    if (
      firstBlockSymbol &&
      firstBlockSymbol === SecondBlockSymbol &&
      firstBlockSymbol === ThirdBlockSymbol
    ) {
    ...
    <>
      <main>
        <div id="game-container">
          ...
          {(winner || gameDraw) && (
            <Gamedraw winner={winner} onRestart={handleStart} />
          )}
